...
if __name__ == '__main__':
    ...
    allo = cP.load(f)
    f.close()
    r_t, und = test.generate_route(allo, 5)
    merge.recal_time(r_t, allo)
    pre_cal = merge.generate_distance_time(r_t, und, allo)
    t1 = time.time()
    opr1 = merge.bb_tsp(r_t, allo, und, pre_cal=pre_cal, append_und=False)
    t2 = time.time()
    opr2, obj = opt_tsp(r_t, und, pre_cal)
    t3 = time.time()
    print(t2-t1)
    print(t3-t2)
